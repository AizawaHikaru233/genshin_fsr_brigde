...
This is not an official project of HoYoverse, AMD, or OptiScaler. Neither the repository nor the Lite ZIP redistributes OptiScaler, FPS Unlocker, or NVIDIA runtime files; the installer obtains them from official sources at runtime or asks the user to select them manually.
...
## Non-Frame-Generation Lite Package

`GenshinOneClick/` contains the Lite installer, the official ReShade Add-on DLL, and HDR shaders. OptiScaler, FPS Unlocker, and NVIDIA DLSS runtime files are excluded from both the repository and package and are obtained only when the installer runs. The non-frame-generation edition selects stable OptiScaler 0.9.3 and locks frame generation off.
...
Release builds keep only basic error logs and overwrite the previous run's logs at startup. After reproducing a problem, retain and submit:
...
- Neither the repository nor the Lite package includes OptiScaler, FPS Unlocker, NVIDIA runtime files, or frame-generation backends; the installer only provides official-source download flows.
