...
> **当前功能边界（请先读）**：只支持**纯纹理贴图类** Mod，**不支持模型类 Mod**。
> 详见下方「支持范围」。
...
以 `hash=` 匹配 + `filename=` 替换**纹理像素内容**的 Mod：

- **DDS**（CPU 路径）：BC1–BC7 / unorm 等，含 DX10 扩展头
- **GDDS**（GPU 路径）：DirectStorage GDeflate 解压直写共享纹理
- 3DMigoto 格式的 `[TextureOverride]` / `[Resource]` ini
- 示例：角色贴图、场景贴图、UI 贴图、特效贴图（如 `Texture++ Finale 4K` 系列）

### ❌ 暂不支持：模型类 Mod

**本组件目前只做"贴图替换"，不改动几何 / 网格 / 骨骼 / 材质定义。**
因此以下类型的 Mod **不会生效**：

| 不支持的类型 | 说明 |
|---|---|
| 模型 / 网格替换（mesh、`.buf`、`.ib`） | 需要 hook 顶点/索引缓冲与绘制调用，本组件不介入 |
| 骨骼 / 动画修改 | 同上 |
| 材质 / 着色器替换 | 需要 hook shader 编译或材质常量 |
| 顶点着色器 / 常量缓冲注入 | 需要额外的管线 hook 面 |
| `[CommandList*]` / `[Present]` 等绘制阶段指令 | 未实现绘制命令重放 |
| 其他非纹理类 ini 段 | 未识别的段会被忽略 |

> 如果你的 Mod 同时包含贴图与模型，**贴图部分会生效，模型部分不会** ——
...
- **显存容量驱动淘汰**：缓存超预算（默认 = 物理显存的 20%）或可用显存低于临界值
  （按容量自适应：4GB→20%、8GB→18.6%、16GB→15.8%、24GB→13%）时，
  按大小优先 + LRU 淘汰**已销毁原纹理**的缓存；未达阈值不清理。
- **渲染线程延迟释放**：非渲染线程不直接 Release D3D11 对象，规避驱动 UAF。

> ⚠️ **仅纹理替换**：以上全部围绕"替换纹理像素内容"。**不支持模型 / 网格 /
> 骨骼 / 材质 / 着色器类 Mod** —— 见上方「支持范围」。
