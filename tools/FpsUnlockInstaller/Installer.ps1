    ...
    if ($null -ne $config) {
        ...
        if ($null -ne $config.DllList) {
            $managedPaths = @{
                ...
                'textureloader.dll' = $textureLoaderPath
            }
...
function Write-InstallCatalog {
    ...
    $textureLoaderVersion = Get-FileVersionLabel -Path $textureLoaderPath
    ...
    $textureLoaderStatus = if ($state.TextureLoader) { '已安装' } else { '未安装' }
    Write-Host ''
...
function Invoke-FoundationSetup {
    ...
    # 安装脚本只负责初始化运行环境（FPS Unlocker + 管理脚本），**默认一个插件都不装**；
    # 装哪些插件一律由用户在菜单里选（"1. 安装模块"）。
    # 首次运行还没有 fps_config.json，Get-ConfiguredPluginState 会把所有模块判为未配置
    # ⇒ 下面逐条追加 -Disable* ⇒ Configure.ps1 写出 DllList 为空，这正是预期结果。
...
function Invoke-InstallWizard {
    ...
    $desired = [ordered]@{
        ...
        HDR = [bool]$state.HDR
        TextureLoader = [bool]$state.TextureLoader
    }
...
function Invoke-UpdateWizard {
    ...
    $componentSelection = @($validSelection | Where-Object { $_ -in @(1, 2, 3, 4, 5) })
...
function Invoke-UninstallWizard {
    ...
    if (5 -in $selection) { Remove-DllFromConfig -Config $config -Paths @($textureLoaderPath) }
