...
struct Signature
{
    ...
    // 可选"首选 RVA"（0 = 未指定）。用于**无法用纯字节模式唯一定位**的函数。
    //
    // `GameObject::SetActive` 就是这种：本镜像里它的模式有 **45 个逐字节完全相同的
    // IL2CPP 包装器**，差别只在 `E8`/`E9` 位移字段里 —— 而位移由**调用者自身地址**
    // 派生（45 个都调用同一个 `m_CachedPtr` 解包助手）⇒ 掩掉则 45 个全中、
    // 保留则等于把地址写死。**这类函数在字节层面就是不可唯一识别的。**
    //
    // 故这里记下地址，但**不盲信**：使用时必须用 `text`（全掩码）在该 RVA 处逐字节
    // 校验；校验不过就记日志并回退到扫描，绝不静默拿错函数。
...
inline constexpr Signature k_signatures[] {
    ...
    },
    {
        ...
        // 而且会**钉在错误的孪生上**。故靠下方 `preferred_rva` 选定，本模式只负责
        // 在该地址处做运行时校验（校验不过会记日志并回退到扫描 —— 见 `resolve` 处）。
        "ObjectActive",
